    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            case (state)
                ...
                START_BIT: begin
                    if (bit_counter == CLKS_PER_HALF_BIT - 1) begin
                        if (!midi_rx) begin
                            state <= DATA_BITS;
                            bit_counter <= 0;
                            bit_index <= 0;
                ...
                DATA_BITS: begin
                    ...
                    else if (bit_counter == CLKS_PER_BIT - 1) begin
                        bit_counter <= 0;
                        if (bit_index == 7)
                            state <= STOP_BIT;
                        else
                ...
                PROCESS_BYTE: begin
                    ...
                    if (rx_data[7] == 1) begin  // Status byte
                        ...
                        if (byte_count == 0) begin
                            ...
                            // Process note immediately after note number
                            if (status_byte == NOTE_ON) begin
                                midi_note <= rx_data[6:0];
                                note_on <= 1;
                                note_valid <= 1;
                            end
                            else if (status_byte == NOTE_OFF) begin
                                midi_note <= rx_data[6:0];
                                note_on <= 0;
                                note_valid <= 1;
                            end
                            ...
                            byte_count <= 0;  // Skip velocity byte
                    end
                end
            endcase
        end
